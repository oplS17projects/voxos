...
(struct spin (layer color frame x y)
  #:methods gen:word
  [(define (word-fps w) 60.0)
   (define (word-label s ft) (lux-standard-label "Voxos" ft))
   ...
   (define (word-event w e)
     ...
     (cond
       ...
       [(and (key-event? e) (eq? (send e get-key-code) #\return))
        (spin-it (add1 layer))
        w]
...
(define (spin-it layer)
  (define s (spin layer 0 0 0 0))
  (fiat-lux s))

(module+ main
  (call-with-chaos (make-gui)
                   (λ () (spin-it 0))))
